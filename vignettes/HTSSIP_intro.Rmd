...
* **taxonomic conservatism of ecological roles**
    * e.g., Are closely related microbes usually performing the same functions in the community?
...
* **HR-SIP**
    * Pepe-Ranney, Charles, Chantal Koechli, Ruth Potrafka, Cheryl Andam, Erin Eggleston, Ferran Garcia-Pichel, and Daniel H. Buckley. 2015. “Non-Cyanobacterial Diazotrophs Mediate Dinitrogen Fixation in Biological Soil Crusts during Early Crust Formation.” The ISME Journal, June. doi:10.1038/ismej.2015.106.
...
* "13C-Glu" and "13C-Cel" stand for 13C-gluclose and 13C-cellulose, respectively
* "Day" is the number of days from substrate addition. 

### Sample metadata

Here's a look at (some) of the sample metadata. Note that **'Bouyant_density'** is
included. This is necessary for many of the analyses in \code{HTSSIP}!
...
For most of the analyses in \code{HTSSIP}, the gradient fraction samples of each labeled-treatment gradient is compared to its corresponding control. For example, the dataset that we've been looking at has 4 comparisons: 
...
Writing a bunch of redundant code to make each subset is not a good method to subset the data if you have a bunch of subsets to make, so \code{HTSSIP} includes some functions to help. 

The expression used in the \code{prune_samples()} to subset the data can be generalized as:

```{r}
ex = "(Substrate=='12C-Con' & Day=='${Day}') | (Substrate=='${Substrate}' & Day == '${Day}')"
```

Now we need the parameters for each individual \code{prune_samples()} call. This can be accomplished with:
...
With a list of phyloseq objects (1 list item per treatment-control comparison), we can use \code{lapply} or a similar function to run analyses on each subset of the dataset. 
