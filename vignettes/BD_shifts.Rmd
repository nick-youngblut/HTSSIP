...
buoyant densities. If so, then the pairwise beta diversity between
...
usually only partially overlap. To deal with this issue, I have taken the approach
...
1. The percent overlap of treatment gradient fractions relative to the control 
are calculated. 
...
As with the [beta diversity ordinations](beta_diversity_ordinations.html), 
we are going to split up the dataset
into individual labeled-treamtent + corresponding unlabeled-control comparisons. 
...
Now, let's just measure BD shift for just 1 subset 
(1 item in the list of phyloseq objects)
...
Note that the treatment that we assessed is the 13C-Cel (13C-labeled cellulose). 

Also note that the \code{sample.x} column is all 12C-Con control samples, while the comparison column (\code{sample.y}) is the treatment gradient fraction samples.
...
ggplot(wmean1, aes(BD_min.x, wmean_dist)) +
  geom_point() +
  labs(x=x_lab, y=y_lab, title='Beta diversity of 13C-Cel\nrelative to 12C-Con') +
  theme_bw() +
...
Now let's run \code{BD_shift()} on all phyloseq objects in our list.
We'll use \code{ldply} for this because it preserves the list names
in the resulting data.frame (list names are assigned to \code{.id} by default).

```{r}
wmean = plyr::ldply(physeq_l, BD_shift)
wmean %>% head(n=3)
```
...
wmean = wmean %>%
  mutate(.id = gsub('.+(13C-[A-z]+).+', '\\1', .id))

# plotting, with facetting by 13C-treatment
ggplot(wmean, aes(BD_min.x, wmean_dist)) +
  geom_point() +
  labs(x=x_lab, y=y_lab, 
       title='Beta diversity of 13C-treatments\nrelative to 12C-Con') +
  theme_bw() +
  theme(text = element_text(size=16)) +
  facet_grid(.id ~ .)
```

As you can see, the 'heavy' beta diversity spike is stronger for 13C-Cel versus
13C-Glu, which you'd probably expect, given that these samples were from
a 14 day incubation with substrate.
