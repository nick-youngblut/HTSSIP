...
Note that the \code{sample.x} column is all 12C-Con control samples, while the comparison column (\code{sample.y}) is the treatment gradient fraction samples.
...
Now let's run \code{BD_shift()} on all phyloseq objects in our list. We'll use \code{ldply} for this because it preserves the list names in the resulting data.frame (list names are assigned to \code{.id} by default).
