...
Quantitative stable isotope probing (q-SIP) deals with the compositional problems of qPCR to acertain the absolute abundances of each OTU in each sample. 
...
`HTSSIP` can both simulate q-SIP datasets and run the q-SIP analysis (with parallel processing of bootstraps). 
...
```{r}
physeq_rep3 %>% otu_table %>% .[1:5, 1:5]
```

```{r}
physeq_rep3_t = OTU_qPCR_trans(physeq_rep3, physeq_rep3_qPCR)
physeq_rep3_t %>% otu_table %>% .[1:5, 1:5]
```
...
We'll use 100 bootstrap replicates. This function can be run in parallel (see the documentation). 
...
ggplot(df_atomX_boot, aes(OTU, A, ymin=A_CI_low, ymax=A_CI_high)) +
  geom_pointrange() +
