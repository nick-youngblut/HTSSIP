...
See [HTSSIP introduction vignette](HTSSIP_intro.Rmd) for a description on why dataset parsing (all treatment-control comparisons) is needed.
...
For clarity, I'm going edit these long strings to make them more readable when plotted.
...
Let's use 3 buoyant density windows (g/ml^-1):
...
You can turn off parallel processing by setting the `parallel` option to `FALSE`. Also, there's 2 different levels that could be parallelized: either the `ldply()` or `HRSIP()`. Here, I'm running HRSIP in parallel, but it may make sence in other situations (eg., many comparisons but few density windows and/or sparsity cutoffs).
...
The density windows can vary for each OTU. Let's plot which density windows were used.
