...
# devtools::test(filter='BD_shift')


#physeq = physeq_l[[1]]
#wmean = BD_shift(physeq)
...
# # metadata
# metadata = format_metadata(physeq)
# head(metadata, n=3) %>% as.data.frame
# # ID overlapping fractions
# metadata_j = fraction_overlap(metadata)
# metadata_j %>% head %>% as.data.frame
# metadata_j %>% select(Fraction.x, Fraction.y,
#                       BD_min.x, BD_max.x,
#                       BD_min.y, BD_max.y)
# # Calculating distances
# physeq_d = phyloseq::distance(physeq,
...
#                    fast=TRUE,
#                    normalized=FALSE)
# physeq_d_p = parse_dist(physeq_d)
#
# # joining dataframes
# physeq_d_p_j = inner_join(physeq_d_p, metadata_j,
#                           c('sample.x'='METADATA_ROWNAMES.x',
#                             'sample.y'='METADATA_ROWNAMES.y'))
# physeq_d_p_j %>% head
#
#
# physeq_d_p_j_m = overlap_wmean_dist(physeq_d_p_j)
#
# ggplot(physeq_d_p_j_m, aes(BD_min.x, wmean_dist)) +
#     geom_point() +
#     theme_bw() +
#     theme(
#         text = element_text(size=16)
